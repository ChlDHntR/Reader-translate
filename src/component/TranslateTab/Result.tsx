...
export default function Result({ searchStatus }: { searchStatus: boolean }) {
  ...
  const [ realResult, setRealResult ] = useState(new ResultClass(result))
  
  useEffect(() => {
    let newResult = new ResultClass(result)
    setRealResult(newResult)
  }, [result])

  return (
    <div className={`h-50 ${result === null ? 'text-center flex justify-center items-center' : ''} overflow-auto`}>
      {searchStatus && <p>Searching...</p>}
      {(realResult.answer !== null) && !searchStatus ? (
        <>
          {realResult.answer.map((item: any, index: number) => (
            <div key={index}>
          ...
          <ResultMonoLang realResult={realResult} />
        </>
      ) : (
